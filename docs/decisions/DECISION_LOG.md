...
## ADR-007 — DeepAgents Stage Resilience: Friendly Errors, One-Retry, Tool-Grounded Answers

- **Date:** 2026-06-14
- **Context:** Three related failures in the DeepAgents pipeline: (1) Gemini occasionally returns a candidate with no `content.parts` (safety filter / MAX_TOKENS / empty turn), making langchain throw `Cannot read properties of undefined (reading 'parts')` — which was pushed RAW to the client, killing the stream; (2) the SSE-parsing `try/catch` in the client provider wrapped BOTH the `JSON.parse` AND the event handling, so the `throw new Error(...)` for an `error` event was immediately swallowed by its own catch — errors never reached the composer, leaving users staring at a raw `{"type":"error",...}` line; (3) the `answer_question` path told the model it "may answer from knowledge", so it hallucinated champions/synergies from a different TFT set than the user's selected version.
- **Decision:** (a) Wrap every stage (router, answer, clarify, plan, propose, builder) in a one-retry helper keyed on `isRecoverableModelError`; streamed stages emit `reset` before re-streaming. Terminal failures throw a single friendly Thai `FRIENDLY_ERROR`; the pipeline catch maps ALL non-friendly exceptions to it too — raw exceptions are logged server-side, never sent to the client. The error event now carries `code: "AI_STREAM_FAILED"`, and `normalizeError` passes that code's backend message through verbatim instead of overwriting it. (b) Split the client provider's parse vs. handle into separate try/catch so a thrown `ApiException` propagates. (c) Router failures default silently to `build_team` (it never streams to the user). (d) The answer prompt now MANDATES tool calls before naming any champion/synergy/item and forbids substituting from memory.
- **Reason:** A transient empty-candidate response should cost one retry, not the whole turn. The nested-catch bug was the actual reason users saw raw error JSON — isolating the parse is the fix, not just nicer messages. Forcing tool grounding is the only reliable cure for cross-version hallucination, since the model's weights encode a different set than the live data.
